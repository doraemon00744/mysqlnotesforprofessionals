...
这类值以二进制格式存储。在这种两列的声明中，数据的精度和规模都必须指定，精度表示存储数字的有效数字的位数，规模表示小数点后面的位数。

```sql
salary DECIMAL(5,2)
```

5表示精度，2表示规模。在上面这个例子中，可以存储的数字的范围从-999.99到999.99。

如果后面那个规模参数省略的话，缺省值是0。

精度参数最大可以达到65，也就是这里最多可以存储65位有效数字的数。

**DECIMAL(M,N)**所占用的空间大概是M/2个字节。